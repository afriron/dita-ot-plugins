...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="*[contains(@class, ' map/topicref ')]
	...
	<xsl:variable name="level">
      <xsl:value-of select="count(ancestor::*[contains(@class, ' map/topicref ')])"/> 
    </xsl:variable>
    ...
    <xsl:choose>
      <xsl:when test="normalize-space($title)">
		<xsl:element name="div">
			...
			<xsl:if test="count(topicref) &gt; 0">
				<xsl:attribute name="class">withchild level<xsl:value-of select="$level"/></xsl:attribute>
			</xsl:if>
          ...
          <xsl:choose>
            ...
            <xsl:when test="normalize-space(@href)">
              ...
              <xsl:element name="a">
				...
				<!--xsl:variable name="fig-count-actual" select="
				<xsl:number count="*[contains(@class,' topic/topic ')][not(@props)]" level="multiple"
				format="1.1" />
